...
def load_content() -> dict:
    ...
    details = json.loads(DETAILS_CONTENT_PATH.read_text(encoding="utf-8")) if DETAILS_CONTENT_PATH.exists() else {}
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            <a href="#home" class="logo" aria-label="{esc(site["name"])} home">
                <span class="logo-mark">{esc(site["initials"])}</span>
            </a>
...
:root {
    /* Resume-inspired orange, purple, white, and grayscale palette. */
    --palette-orange: #e94f2f;
    --palette-purple: #7352a8;
    --palette-lilac: #b9a7d8;
    ...
    --accent-dark: var(--palette-purple);
    --highlight: var(--palette-lilac);
    --gradient: linear-gradient(110deg, var(--palette-orange), var(--palette-purple));
...
.hero::before {
    ...
    right: -12vw;
    border-radius: 50%;
    background: radial-gradient(circle, color-mix(in srgb, var(--palette-purple) 12%, transparent), transparent 68%);
    pointer-events: none;
}
...
.project-card:hover {
    ...
    border-color: var(--accent);
    box-shadow: 0 12px 30px color-mix(in srgb, var(--palette-purple) 12%, transparent);
}

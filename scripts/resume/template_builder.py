...
def create_resume_template(output_path: Path) -> None:
    ...
    _add_page_one(document)
    document.add_paragraph().add_run().add_break(WD_BREAK.PAGE)
    _add_page_two(document)
...
def _add_page_one(document: Document) -> None:
    ...
    _add_education_line(document, "EDU")
    _add_education_line(document, "EDU2")

    _section_heading(document, "CORE CAPABILITIES")
    _add_skill_grid(document)

    _section_heading(document, "EXPERIENCE")
    _add_entry(document, "EXP", 1, 4)
    _add_entry(document, "EXP", 2, 4)

    _section_heading(document, "LEADERSHIP")
    _add_entry(document, "LEAD", 1, 1)
    _add_entry(document, "LEAD", 2, 1)

    _section_heading(document, "COMMUNITY INVOLVEMENT")
    _add_entry(document, "COMM", 1, 1)

    _section_heading(document, "RECOGNITION")
    _add_entry(document, "RECOG", 1, 1)
    _add_entry(document, "RECOG", 2, 1)
    _add_entry(document, "RECOG", 3, 1)


def _add_education_line(document: Document, tag_prefix: str) -> None:
    ...
    _add_control(bullet, f"{tag_prefix}_BULLET1", "Education detail", size=8.4)


def _add_page_two(document: Document) -> None:
    header = document.add_paragraph()
    _compact(header, after=7)
    _add_control(header, "PAGE2_NAME", "Name", color=NAVY, bold=True, size=14)
    _add_static(header, "  |  SELECTED PROJECT PORTFOLIO", bold=True, size=10)
    ...
    _section_heading(document, "SELECTED PROJECT PORTFOLIO")
    _add_entry(document, "PROJECT", 1, 4)
    _add_entry(document, "PROJECT", 2, 4)
    _add_entry(document, "PROJECT", 3, 4)
    _add_entry(document, "PROJECT", 4, 4)
...
def _add_entry(document: Document, prefix: str, index: int, bullets: int) -> None:
    ...
    _compact(title, before=4, after=0)
...
def _add_technical_skills(document: Document) -> None:
    ...
    for row, (label, tag) in zip(table.rows, rows, strict=True):
        ...
        for cell in row.cells:
            cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
        _set_cell_fill(label_cell, "E9EEF2")
...
def _set_cell_fill(cell, colour: str) -> None:

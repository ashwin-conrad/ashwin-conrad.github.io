...
The public Word template remains fixed at two pages. Its explicit ID-based slot policy is stored in `resume.json > _meta.word_slot_order` and read by `scripts/resume/mapper.py`; no résumé-specific IDs are embedded in Python. The working projection expands that policy with one blank add-new slot per repeatable section and one trailing blank bullet per populated entry. Word sync assigns a new neutral sequence ID when an add-new slot is filled, then persists that ID in both the section JSON and slot policy. A fresh working rebuild always adds the next blank capacity.
...
`content/resume.json` remains the manifest plus neutral slot selection for the public layout. Its section files under `content/details/resume/` are recombined before the resume mapper runs. Project records may carry a `category`; the page-two mapper emits a category label at the start of each consecutive group while preserving the project order configured in `word_slot_order.projects`. The working DOCX may contain empty add-new entries and trailing bullet controls, but generated `portfolio/resume.docx` removes those unused controls before PDF conversion. Filled controls are stored as ordinary JSON fields and can grow beyond the original bullet counts; the next working rebuild adds fresh blank capacity. The first-page order is Experience, Leadership, Community Involvement, then Recognition. Run the CLI quality check after source edits.
